-- NOT YET APPLIED (2026-09-22). Proof that the vinegar came out on time.
-- The removal time is stamped by the SERVER (now()), never sent by the phone,
-- and a photo is required. The marination time stays editable (it may be
-- recorded after the fact); the removal time cannot be.

alter table public.chicken_marinations
  add column if not exists unload_photo_url text;

comment on column public.chicken_marinations.unloaded_at is
  'Server-stamped when the removal was recorded. Never accepted from the client, so it cannot be back-dated.';

-- Replace the earlier version, which trusted a client timestamp and took no photo.
